    ...
    gm.unit.forEach((u) => {
      ...
      if (u.team == number) {
        if (u.tp == "firebat") {
          ...
          if (u.akt.length > 0) {
            let zakt = () => {
              let arr = u.akt.filter((a) => {
                return (
                  a.img == "firebat" &&
                  en.unitInPoint(gm, a.x, a.y).team != number &&
                  en.unitInPoint(gm, a.x, a.y).team != 3
                );
              });
              return arr;
            };
            ...
            if (zakt().length > 0) game.order(gm, u, _.sample(zakt()));
            else if (makt().length > 0) {
              game.order(gm, u, makt()[0]);
              if (zakt().length > 0) game.order(gm, u, _.sample(zakt()));
            } else game.order(gm, u, _.sample(u.akt));
          }
        } else if (u.tp == "teleporter") {
          let tarr = u.akt.filter((a) => {
            return a.img == "teleporter";
          });
          if (tarr.length > 0) game.order(gm, u, _.sample(tarr));
          else if (u.akt.length > 0) game.order(gm, u, _.sample(u.akt));
        } else if (u.tp == "worm") {
          if (u.akt.length > 0) {
          ...
          if (u.akt.length > 0) game.order(gm, u, _.sample(u.akt));
        }
      }
    });
    gm.gold[number - 1] += 10;
